...
vec3 shadeMaterialShaderGI(inout Ray ray, inout vec3 throughput) {
    ...
    // Sample one random light and multiply by lightCount for unbiased estimate
    Illumination illum = illuminate(ray, getRandomLight());
    float cosine = max(dot(-illum.direction, normal), 0.0);

    // Diffuse term
    fragmentColor += diffuseCoefficient * diffuseColor * illum.color * cosine * lightCount;

    // Specular term
    float specCosine = dot(-illum.direction, reflection);
    if (specCosine > 0.0) {
        fragmentColor += specularCoefficient * specularColor * illum.color * pow(specCosine, shininessExponent) * lightCount;
    }

    // Decide continuation: reflection or diffuse indirect
