...
layout(binding = 53, std430) buffer BrdfDataBuffer {
    float brdfData[];
};
...
vec3 lookup_brdf_values(float theta_in, float phi_in, float theta_out, float phi_out, int brdfId) {
    int baseOffset = getBrdfDataOffset(brdfId);
